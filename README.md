...
### Running the Plugin Corpus
The plugins under `plugins/` exercise the *JungleGym* dataset in `dataset/`, which pulls in more third-party frameworks and sanitization libraries than `taintmonkey` itself needs. Install those separately.

```
pip install -r requirements-dataset.txt
```

One of them, `python-magic`, wraps the `libmagic` C library, which is installed at the OS level rather than via pip.
...
With both installed, run the full plugin corpus the same way as the unit tests.

```
PYTHONPATH=. pytest plugins/
```
